...
function activaTablaCotizaciones() {
...
function activaTablaPartidasCotizacion() {

                $("#tablaPartidasCotizacion").DataTable({
      "language": {
...
$(document).on("click", ".btnVerPartidasCotizacion", function(){

        //$(".close").hide();

        $("#buscarCotizaciones").attr("teclaEsc", "no");

        var id_cotizacion = $(this).attr("id_cotizacion");

var datos =  {"id_cotizacion": id_cotizacion};


$.ajax({
        data:datos,
        type: 'POST',
        url: 'vistas/modulos/consultas/consultaVerPartidasCotizacion.php',
        success: function(data) {

        $("#modalVerPartidasCotizacion").modal("show");

        document.getElementById("incrustarTablaPartidasCotizacion").innerHTML = data;
        activaTablaPartidasCotizacion();
        }
        });
...
function buscarAhoraCotizaciones(buscarCotizaciones) {

    document.getElementById("incrustarTablaCotizaciones").innerHTML = "";

                var parametros = {"buscarCotizaciones":buscarCotizaciones};
                $.ajax({
                        data:parametros,
                        type: 'POST',
                        url: 'vistas/modulos/buscadores/buscadorCotizaciones.php',
                        success: function(data) {
                                document.getElementById("incrustarTablaCotizaciones").innerHTML = data;

                                activaTablaCotizaciones();
                        }
                });
        }
...
$(document).keydown(function(event) {
    if (event.which === 27)
    {
        ...
        var buscador_esc = $("#buscarCotizaciones").attr("teclaEsc");
        if(buscador_esc == "si"){
        $("#buscarCotizaciones").val("");
            $("#buscarCotizaciones").focus();
        }else{
        $(".close").trigger('click');
        
        $("#buscarCotizaciones").attr("teclaEsc", "si");
...
$(document).keydown(function(event) {
    if (event.which === 112)
    {
        event.preventDefault();
        $(".close").trigger('click');
        $("#btnCrearNuevaCotizacion").trigger('click');
        //$(".close").hide();
        $("#buscarCotizaciones").attr("teclaEsc", "no");
...
$(document).keydown(function(event) {
    if (event.which === 115)
    {
        ...
        $(foco).parent().parent().children(".botones").children(".btn-group").children(".btnVerPartidasCotizacion").trigger("click"); 
