...
console.log(agentes);
        ...
        function buscarAhoraVentasGarantias(buscarVentasGarantias) {

                var parametros = {"buscarVentasGarantias":buscarVentasGarantias};
                $.ajax({
                        data:parametros,
                        type: 'POST',
                        url: 'vistas/modulos/buscadores/buscadorVentasGarantias.php',
                        success: function(data) {
                                document.getElementById("incrustarTablaVentasGarantias").innerHTML = data;
                        }
                });
        }
...
=============================================*/
$(document).on("click", ".btnSeleccionaVentaGarantia", function(){

        var foo = document.getElementById("a");
        var id_venta = $(this).attr("id_venta");

        $("#idVentaGarantiaSeleccionada").val(id_venta);
        ...
        var parametros = {"id_venta":id_venta};
                $.ajax({
                        data:parametros,
                        type: 'POST',
                        url: 'vistas/modulos/consultas/ventaGarantiaSeleccionada.php',
                        success: function(data) {
                ...
                $("#buscarVentasGarantias").val("");

                $("#buscarVentasGarantias").onkeyup(buscarAhoraVentasGarantias($('#buscarVentasGarantias').val()));
...
$(document).keydown(function(event) {
    if (event.which === 27)
    {
        ...
        var buscador_esc = $("#buscarVentasGarantias").attr("teclaEsc");
        if(buscador_esc == "si"){
        $("#buscarVentasGarantias").val("");
            $("#buscarVentasGarantias").focus();
        }else{
        $(".close").trigger('click');
        
        $("#buscarVentasGarantias").attr("teclaEsc", "si");
...
$(document).keydown(function(event) {
    if (event.which === 112)
    {
        ...
        $("#btnConfirmarGarantia").trigger('click');
    }
});
...
$(document).keydown(function(event) {
    if (event.ctrlKey && event.which === 38)
    {
        ...
        $(".formularioGarantia").on("change", "input.ingresoADevolver", function(){
        ...
        $("#nuevoTotalGarantia").val(sumaTotalPrecio);
        $("#totalGarantia").val(sumaTotalPrecio);
        $("#textoTotal").text(sumaTotalPrecio);
        $("#nuevoTotalGarantia").attr("total",sumaTotalPrecio);
        ...
        function listarProductosGarantia(){
                ...
                $("#listaProductosGarantia").val(JSON.stringify(listaProductos)); 
...
=============================================*/
function validarIdMotivoGarantiaVacio() {
if($("#nuevoIdMotivoGarantia").val() === ""){
...
$(this).blur();
       ...
       Swal.fire({
            ...
            if(result.value){
                ...
                var totalGarantia = $("#totalGarantia").val();
                ...
                if(totalGarantia == 0){
                        ...
                        validar_id_motivo_garantia_vacio = validarIdMotivoGarantiaVacio();



                        if(validar_id_motivo_garantia_vacio !== 0){
    
                                document.forms["formularioGarantia"].submit();
                        }
