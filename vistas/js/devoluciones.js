...
console.log(agentes);
        ...
        function buscarAhoraVentasDevoluciones(buscarVentasDevoluciones) {

                var parametros = {"buscarVentasDevoluciones":buscarVentasDevoluciones};
                $.ajax({
                        data:parametros,
                        type: 'POST',
                        url: 'vistas/modulos/buscadores/buscadorVentasDevoluciones.php',
                        success: function(data) {
                                document.getElementById("incrustarTablaVentasDevoluciones").innerHTML = data;
                        }
                });
        }
...
=============================================*/
$(document).on("click", ".btnSeleccionaVentaDevolucion", function(){

        var foo = document.getElementById("a");
        var id_venta = $(this).attr("id_venta");

        $("#idVentaDevolucionSeleccionada").val(id_venta);

        if (foo.children.length > 0) {

                Swal.fire({
                  title: 'Estas segur@?',
                  text: "Quieres seleccionar esta nueva venta?",
                  footer: 'Si la confirmas los datos de la venta actual desapareceran',
                  icon: 'warning',
        ...
        var parametros = {"id_venta":id_venta};
                $.ajax({
                        data:parametros,
                        type: 'POST',
                        url: 'vistas/modulos/consultas/ventaDevolucionSeleccionada.php',
                        success: function(data) {
                ...
                $("#buscarVentasDevoluciones").val("");

                $("#buscarVentasDevoluciones").onkeyup(buscarAhoraVentasDevoluciones($('#buscarVentasDevoluciones').val()));
...
$(document).keydown(function(event) {
    if (event.which === 27)
    {
        ...
        var buscador_esc = $("#buscarVentasDevoluciones").attr("teclaEsc");
        if(buscador_esc == "si"){
        $("#buscarVentasDevoluciones").val("");
            $("#buscarVentasDevoluciones").focus();
        }else{
        $(".close").trigger('click');
        
        $("#buscarVentasDevoluciones").attr("teclaEsc", "si");
...
$(document).keydown(function(event) {
    if (event.ctrlKey && event.which === 38)
    {
        ...
        $(".formularioDevolucion").on("change", "input.ingresoADevolver", function(){
                ...
                var ingresoCantidadVendida = $(this).parent().parent().children(".divCantidadVendida").children(".ingresoCantidadVendida");

                var cantidad_vendida = ingresoCantidadVendida.val();
                ...
                for(var i = 0; i < id_producto.length; i++){
                    ...
                    if($(cantidad[i]).val() > 0){
                        ...
                        listaProductos.push({ "id_producto" : $(id_producto[i]).attr("id_producto"),
                             "id_partvta" : $(id_producto[i]).attr("id_partvta"),
    ...
    $(document).on("click", "#devolverVentaEntera", function(){

        var listaProductos = [];

        var id_producto = $(".nuevaClaveProducto");

        var cantidad = $(".ingresoADevolver");

        var cantidad_devuelta = $(".ingresoCantidadTotalDevuelta");

        var cantidad_vendida = $(".ingresoCantidadVendida");

        if($("#devolverVentaEntera").is(":checked")){

            for(var i = 0; i < id_producto.length; i++){

                var cantidad_disponible = $(cantidad_vendida[i]).val() - $(cantidad_devuelta[i]).val();

                $(cantidad[i]).val(cantidad_disponible).trigger("change");
...
function validarTipoDevolucion() {
if($("#nuevoTipoDevolucion").val() === ""){
        
        Swal.fire({
        icon: 'error',
        title: 'Debe seleccionar el tipo devolución',
        text: 'Efectivo o Físico',
        showConfirmButton: true
        }).then(function(result){
            $("#nuevoTipoDevolucion").focus();
        });
       ...
       Swal.fire({
            ...
            if(result.value){
                ...
                if(totalDevolucion == 0){
                        ...
                        validar_tipo_devolucion =  validarTipoDevolucion();

                        if(validar_id_motivo_devolucion_vacio !== 0 && validar_tipo_devolucion !== 0){
