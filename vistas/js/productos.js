function stringTitleCase(_string) {
 var splitString = _string.toLowerCase().split(' ');
...
$(document).on("click", ".btnEstatus", function(){
    ...
    datos.append("actualizarProductoES", id_producto);
    datos.append("columna", columna);
    datos.append("valor", estatusProducto);
...
function buscarAhoraProductosD(buscarProductosD) {
    ...
    var texto = stringTitleCase(buscarProductosD);
    $("#buscarProductosD").val(texto);
...
$(document).keydown(function(event) {
    if (event.which === 27)
    {
        ...
        if(buscador_esc == "si"){
            ...
            $(foco).parent().parent().parent().parent().children(".contador"+contador_actual).children().children().children(".btnEditarProducto").addClass('foco').focus();
        }
    }
});
...
$(document).keydown(function(event) {
    //CAMBIAR A LAS ANTERIORES IMAGENES DE UN PRODUCTO
    if (event.ctrlKey && event.which === 38)
    {
        ...
        if(buscador_esc == "no"){
            ...
            //alert("contador mas: "+contador_mas);
            //alert("contador menos: "+contador_menos);
