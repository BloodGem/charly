...
$(document).on("click", ".btnEditarCliente", function(){
  ...
  var id_cliente = $(this).attr("id_cliente");
    ...
    $("#buscarClientes").attr("teclaEsc", "no");
...
$(document).on("click", ".btnEliminarCliente", function(){
  ...
  confirmButtonText: 'Si'
    ...
    if(result.value){
      ...
      window.location = "index.php?ruta=lista-clientes&id_cliente="+id_cliente;
...
function buscarAhoraClientes(buscarClientes) {

    document.getElementById("incrustarTablaClientes").innerHTML = "";

        var parametros = {"buscarClientes":buscarClientes};
        $.ajax({
        data:parametros,
        type: 'POST',
        url: 'vistas/modulos/buscadores/buscadorClientes.php',
        success: function(data) {
        document.getElementById("incrustarTablaClientes").innerHTML = data;
        activaTablaClientes();
        }
        });
        }
...
$(document).keydown(function(event) {
    if (event.which === 27)
    {
        ...
        var buscador_esc = $("#buscarClientes").attr("teclaEsc");
        if(buscador_esc == "si"){
        $("#buscarClientes").val("");
        ...
        $("#buscarClientes").attr("teclaEsc", "si");
...
$(document).keydown(function(event) {
    if (event.which === 112)
    {
        event.preventDefault();
        $(".close").trigger('click');
        $("#btnCrearNuevoCliente").trigger('click');
        //$(".close").hide();
        $("#buscarClientes").attr("teclaEsc", "no");
...
$(document).keydown(function(event) {
    if (event.which === 113)
    {
        ...
        $(foco).parent().parent().children(".botones").children(".btn-group").children(".btnEditarCliente").trigger("click"); 
...
$(document).keydown(function(event) {
    if (event.which === 114)
    {
        ...
        $(foco).parent().parent().children(".botones").children(".btn-group").children(".btnEliminarCliente").trigger("click"); 
...
function validarContactoVacioEditar() {
    ...
    if($("#editarContacto").val() === ""){
        ...
        Swal.fire({
        icon: 'error',
        title: 'Debe introducir el contacto del cliente',
        showConfirmButton: false,
        timer: 2000
        });
...
function validarTelefono1VacioEditar() {
    ...
    if($("#editarTelefono1").val() === ""){
        ...
        Swal.fire({
        icon: 'error',
        title: 'Debe introducir un número telefónico del cliente',
        showConfirmButton: false,
        timer: 2000
        });
...
$(document).on("click", "#btnEditarCliente", function(){
    ...
    document.forms["formularioEditarCliente"].submit();
