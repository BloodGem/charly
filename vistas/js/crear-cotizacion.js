...
function buscar_ahora3(buscar3) {
    ...
    if(buscar3 == ""){
        ...
        $.ajax({
            data:parametros,
            type: 'POST',
            url: 'vistas/modulos/buscadores/buscadorProductosCotizaciones.php',
...
$(document).keydown(function(event) {
    ...
    if (event.which === 119)
    {
        ...
        $("#btnGenerarCotizacion").trigger('click');
...
$(document).on("change", "#seleccionarTipoCotizacion", function(){

    var id_cliente = $("#nuevoIdCliente2").val();

    if(id_cliente == 1){
        $("#seleccionarTipoCotizacion option:eq(0)").removeAttr("selected");
        $("#seleccionarTipoCotizacion option:eq(0)").attr("selected","selected");
    }
});
...
var contador = 0;
        ...
        VER AUTOS
        =============================================*/
$(".listaProductosCotizaciones tbody").on("click", "button.verDetallesProducto", function(){
...
/*(document).on("click", "#inputInserteCantidad", function(){   
        ...
        =============================================*/
var myArr = [];
$(".listaProductosCotizaciones tbody").on("click", "button.agregarProducto", function(){
            ...
            if(es_compuesto == 0){
                ...
                if(stock > 0){
                    ...
                    //var no_precio = $("#nuevoIdCliente2").attr("no_precio");
                        ...
                        var descuento = $("#nuevoDescuentoCotizacion").val();
...
$("#nuevoIdCliente").on('change', function() {
    ...
    if(result.value){
        ...
        $("#nuevoDescuentoCotizacion").val(0);

        var id_cliente = $("#nuevoIdCliente>option:selected").val();

        if(id_cliente == 1){
            $("#nuevoTipoCotizacion option:eq(0)").removeAttr("selected");
            $("#nuevoTipoCotizacion option:eq(0)").attr("selected","selected");
        }
...
$(".listaProductosCotizaciones").on("draw.dt", function(){
    console.log("tabla")
})
...
$(".formularioCotizacion").on("click", "button.quitarProducto", function(){
    ...
    if($(".nuevoProducto").children().length == 0){
        ...
        $("#nuevoTotalCotizacionComision").attr("total",0);
        $("#nuevoTotalCotizacionComision").val(0);
        $("#totalCotizacionComision").val(0);
        $("#nuevoImpuestoCotizacionComision").val(0);
...
$(".formularioCotizacion").on("click", "input.nuevaCantidadProducto", function(){
...
function listarProductos(){
    ...
    for(var i = 0; i < cantidad.length; i++){
        ...
        listaProductosCotizacion.push({ "id" : $(cantidad[i]).attr("id_producto"),
    ...
    //console.log("listaProductosCotizacion", listaProductosCotizacion);

    $("#listaProductosCotizacion").val(JSON.stringify(listaProductosCotizacion)); 
  ...
  if ($(this).is(':checked')) {
    ...
    $(this).attr('value', 'false');
        ...
        IMPRIMIR NOTA
        =============================================*/
$(document).on("click", "#btnGenerarCotizacion", function(){


    var productos = $("#listaProductosCotizacion").val();

    var dataVEPV = new FormData();
    dataVEPV.append("verificarExistenciasProductosCotizacion", productos);
    ...
    $.ajax({
        ...
        success:function(respuestaVEPV){
            ...
            if(json_id_producto.length == 0){
                Swal.fire({
                    title: 'Estas segur@?',
                    text: "Quieres generar la cotización?",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                                                                    ...
                                                                    success:function(respuestaVendedor){
                                                                        ...
                                                                        if(respuestaVendedor == false){
                                                                            ...
                                                                            Swal.fire({
                                                                                ...
                                                                                text: 'No se puede finalizar cotización',
                                                                                showConfirmButton: true
                                                                            });
                                                                            ...
                                                                            setTimeout(function() {
                                                                                document.forms["formularioCotizacion"].submit();
                                                                            }, 100);
                                                                        }
                                                                    }
                                        }//Respuesta vendedor
    ...
    if(input_no_precio.attr("readonly") !== undefined){
        Swal.fire({
          ...
          preConfirm: () => {
            ...
            var datos = new FormData();
            datos.append("darPermisoCodigo", codigo);
            datos.append("darPermisoPermiso", "Desbloquear lista de precios en crear cotizaciones");
