...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->SetY(10);
        $this->SetX(50);
        $this->Cell(0, 15, 'Compras de tipo factura '.$texto, 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->SetY(17);
        $this->SetX(50);
    ...
    public function exportarPDFReporteComprasTipoFactura(){
...
$sql = "SELECT DATE(fecha_creacion) AS dia, SUM(total) AS total_compras FROM compras WHERE fecha_creacion BETWEEN '$fecha1' AND '$fecha2'".$sql_id_sucursal." AND estatus = 1 AND tipo_compra = 1 GROUP BY DATE(fecha_creacion)";

$traerDiasCompras = ControladorReportesCompras::ctrMostrarRegistros($sql);
...
foreach ($traerDiasCompras as $key => $row) {
$bloque1 = <<<EOF


    <table style=style="border: 1px solid black; font-size:12px;">
<tr>
<td style="border: 1px solid black; width:635px; text-align:center; font-weight:bold;">FACTURADO $row[dia]</td>

</tr>
<tr>
<td style="border: 1px solid black; width:40px; text-align:center; font-weight:bold;">No. Compra</td>
<td style="border: 1px solid black; width:125px; text-align:center; font-weight:bold;">Fecha</td>
<td style="border: 1px solid black; width:80px; text-align:center; font-weight:bold;">No. Factura</td>
<td style="border: 1px solid black; width:300px; text-align:center; font-weight:bold;">Proveedor</td>
...
// ---------------------------------------------------------
    ...
    $sql2 = "SELECT id, fecha_creacion, no_factura, id_proveedor, total FROM compras WHERE fecha_creacion BETWEEN '".$row['dia']." 00:00:00' AND '".$row['dia']." 23:59:59'".$sql_id_sucursal." AND estatus = 1 AND tipo_compra = 1";


        $traerCompras = ControladorReportesCompras::ctrMostrarRegistros($sql2);



    foreach ($traerCompras as $key2 => $row2) {  

        $traerProveedor = ControladorProveedores::ctrMostrarProveedor($row2['id_proveedor']);
...
$bloque2 = <<<EOF
    ...
    <table style="border: 1px solid black; font-size:11px;">
        ...
        <tr>
            ...
            <td style="border: 1px solid black; width:40px; text-align:center;">$row2[id]</td>

            <td style="border: 1px solid black; width:125px; text-align:center;">$row2[fecha_creacion]</td>
            
            <td style="border: 1px solid black; width:80px; text-align:center;">$row2[no_factura]</td>

            <td style="border: 1px solid black; width:300px; text-align:center;">$traerProveedor[nombre]</td>
...
$bloque3 = <<<EOF
    ...
    <table style=style="border: 1px solid black; font-size:12px;">
<tr>
<td style="border: 1px solid black; width:545px; text-align:right; font-weight:bold;">TOTAL</td>
<td style="border: 1px solid black; width:90px; text-align:right; font-weight:bold;">$$total_compras</td>
...
if($id_sucursal !== "0"){
    $pdf->Output('compras tipo factura.pdf', 'D');
}else{
    $pdf->Output('compras tipo factura.pdf', 'D'); 
}
...
$compras_sucursales = new MYPDF();
$compras_sucursales -> no_rango = $_GET["no_rango"];
$compras_sucursales -> id_sucursal = $_GET["id_sucursal"];
$compras_sucursales -> tipo = $_GET["tipo"];
if(isset($_GET['rango_fecha'])){
    $compras_sucursales -> rango_fecha = $_GET['rango_fecha'];
}
$compras_sucursales -> exportarPDFReporteComprasTipoFactura();
